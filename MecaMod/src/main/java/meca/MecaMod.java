...
import meca.KeyEvent.ClientProxy;
import meca.KeyEvent.CommonProxy;
import meca.KeyEvent.MessageKeyPressed;
import meca.KeyEvent.PacketHandler;
...
public class MecaMod {
	...
	@SidedProxy(clientSide = "meca.KeyEvent.ClientProxy", serverSide = "emca.KeyEvent.CommonProxy")
	...
	public void KeyHandlingEvent(KeyInputEvent event) {
			if (ClientProxy.keyBindings1.isPressed()) {
				PacketHandler.INSTANCE.sendToServer(new MessageKeyPressed((byte) 1));//1をサーバーに送る。
			}
